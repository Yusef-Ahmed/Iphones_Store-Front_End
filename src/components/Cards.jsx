...
export function Cards({ products, type }) {
  return (
    <div className="px-4 py-12 mx-8">
      <div className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-10">
        {products.map((product) => (
          <div
            ...
            className="flex flex-col transition duration-300 will-change-transform group delay-0 hover:-translate-y-1 hover:scale-110"
          >
            {/*               Product image               */}
            <Link
              to={"/products/" + product.ProductID}
              className="w-full h-2 overflow-hidden rounded-lg aspect-h-1 aspect-w-1"
            >
              <img
                alt="product image"
            ...
            <section className="flex flex-col justify-between mt-5 px-2">
              {/*               Title               */}
              <h3 className="text-sm overflow-hidden h-10 mb-4">
                {product.ProductTitle}
              </h3>
              {/* market & sellerAnalysis & fav button */}
              {type != "matched" && (
                <AnimatedHeart
                  type={type}
                  id={product.ProductID}
                  status={product.inWishList}
                />
              )}
      ...
      {!products.length && (
        <h1 className="flex justify-center text-7xl mt-12">No results found</h1>
      )}
    </div>
  );
}
